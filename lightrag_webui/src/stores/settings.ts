...
type Tab = 'documents' | 'knowledge-graph' | 'retrieval' | 'api' | 'data-analysis' | 'mind-map' | 'platform'
...
interface SettingsState {
  ...
  // Entity Icon Mapping
  entityIconMapping: Record<string, string>
  setEntityIconMapping: (mapping: Record<string, string>) => void
  updateEntityIconMapping: (newMapping: Record<string, string>) => void

  // Saved Entity Types List (for Upload Dialog)
  savedEntityTypes: {name: string, icon: string}[]
  setSavedEntityTypes: (types: {name: string, icon: string}[]) => void
}
...
const useSettingsStoreBase = create<SettingsState>()(
  persist(
    (set) => ({
      ...
      setEntityIconMapping: (mapping) => set({ entityIconMapping: mapping }),
      updateEntityIconMapping: (newMapping) =>
        set((state) => ({
          entityIconMapping: { ...state.entityIconMapping, ...newMapping },
        })),
      
      savedEntityTypes: [
        { name: '人物', icon: 'person' },
        { name: '地点', icon: 'location' },
        { name: '组织', icon: 'organization' }
      ],
      setSavedEntityTypes: (types) => set({ savedEntityTypes: types }),
      ...
      addWorkspace: (workspace: string) =>
        set((state) => {
          ...
          if (state.workspaces.includes(normalized)) return { currentWorkspace: normalized }
          return {
      ...
      removeWorkspace: (workspace: string) =>
        set((state) => {
          const next = state.workspaces.filter((w) => w !== workspace)
          const nextCurrent = state.currentWorkspace === workspace ? null : state.currentWorkspace
          return { workspaces: next, currentWorkspace: nextCurrent }
        }),
    ...
    }),
    {
      ...
      migrate: (state: any, version: number) => {
        ...
        if (version < 20) {
          ...
          if (typeof state.currentWorkspace !== 'string') {
            state.currentWorkspace = null
          }
        }
        if (version < 21) {
          if (state.querySettings && typeof state.querySettings.enable_thinking !== 'boolean') {
            state.querySettings.enable_thinking = false
          }
        }
        return state
      }
