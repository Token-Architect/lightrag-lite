...
const hexToRgba = (hex: string, alpha: number): string => {
  ...
  if (clean.length !== 6) return `rgba(120,138,165,${alpha})`
...
const createSigmaGraph = (rawGraph: RawGraph | null) => {
  ...
  for (const rawNode of rawGraph?.nodes ?? []) {
    ...
    const entityType = rawNode.properties?.entity_type;
    const nodeSize = Math.max(7, Number(rawNode.size || 7))
    const borderColor = hexToRgba(rawNode.color || DEFAULT_NODE_COLOR, 0.75)
    const primaryLabel = rawNode.labels?.[0] || rawNode.id

    // Log for debugging: check first few nodes to see their structure
    if (Math.random() < 0.05) { // Only log ~5% of nodes to avoid spamming console
    ...
    graph.addNode(rawNode.id, {
      ...
      // for node-border
      borderColor,
      borderSize: Math.max(1.2, nodeSize * 0.09),
      forceLabel: nodeSize >= 13,
      zIndex: nodeSize >= 15 ? 1 : 0,
      type: 'border'
    })
  }
...
const useLightrangeGraph = () => {
  ...
  const entityIconMapping = useSettingsStore.use.entityIconMapping()
  ...
  // Re-create sigma graph when entityIconMapping changes
  useEffect(() => {
    if (rawGraph && !isFetching) {
      console.log('Updating sigma graph due to icon mapping change');
      const graph = createSigmaGraph(rawGraph);
      if (graph) {
        useGraphStore.getState().setSigmaGraph(graph);
      }
    }
  }, [entityIconMapping, rawGraph, isFetching]);
