...
const GraphViewer = () => {
  ...
  // Use store actions to trigger graph refresh instead of direct fetch
  const setGraphDataFetchAttempted = useGraphStore(state => state.setGraphDataFetchAttempted)
  const incrementGraphDataVersion = useGraphStore(state => state.incrementGraphDataVersion)
  ...
  // Initialize data on mount and when tab becomes active
  useEffect(() => {
    // Only fetch if this tab is active
    if (currentTab === 'knowledge-graph') {
      // Trigger data fetch via store state changes that useLightragGraph hook listens to
      setGraphDataFetchAttempted(false)
      incrementGraphDataVersion()
    }
  }, [currentTab, setGraphDataFetchAttempted, incrementGraphDataVersion])
  ...
  return (
    <div className="relative h-full w-full overflow-hidden">
      <SigmaContainer
        ...
        className="ve-graph-canvas !size-full overflow-hidden"
        ref={sigmaRef}
      >
        <div className="ve-graph-atmosphere pointer-events-none absolute inset-0 z-0" />
        <GraphControl />
        ...
        <div className="ve-graph-toolbar absolute bottom-2 left-2 z-10 flex flex-col rounded-2xl border">
          <LayoutsControl />
          <ZoomControl />
          <FullScreenControl />
          <LegendButton />
        ...
        {showLegend && (
          <div className="absolute bottom-10 right-2 z-0">
            <Legend className="ve-graph-legend" />
          </div>
        )}
