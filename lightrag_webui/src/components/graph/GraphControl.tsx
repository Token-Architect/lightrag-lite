  ...
  useEffect(() => {
    ...
    setSettings({
      ...
      nodeReducer: (node, data) => {
        ...
        if (!disableHoverEffect) {
            ...
            try {
              if (node === _focusedNode || graph.neighbors(_focusedNode).includes(node)) {
                ...
                if (node === selectedNode) {
                  ...
                  newData.borderSize = Math.max(2.4, Number((data as any).borderSize || 1.4) + 1.2)
                  newData.size = Number(data.size || 8) * 1.12
                  newData.zIndex = 2
                } else {
                  newData.borderColor = Constants.nodeBorderColorFocused
                  newData.borderSize = Math.max(1.8, Number((data as any).borderSize || 1.2) + 0.45)
                  newData.size = Number(data.size || 8) * 1.05
                  newData.zIndex = 1
                }
              }
            } catch (error) {
          ...
          if (newData.highlighted) {
            ...
            newData.borderColor = Constants.nodeBorderColorMuted
            newData.borderSize = Math.max(1, Number((data as any).borderSize || 1.2) - 0.2)
          }
        }
        return newData
      },
      ...
      edgeReducer: (edge, data) => {
        ...
        const newData = { ...data, hidden: false, labelColor, color: edgeColor as string }
          ...
          if (_focusedNode && graph.hasNode(_focusedNode)) {
            try {
              if (hideUnselectedEdges) {
                ...
                if (graph.extremities(edge).includes(_focusedNode)) {
                  ...
                  newData.size = Number(data.size || 1) * 1.25
                } else {
                  newData.color = Constants.edgeColorMuted
                }
              }
            } catch (error) {
            ...
            if (_selectedEdge || _focusedEdge) {
              if (edge === _selectedEdge) {
                ...
                newData.size = Number(data.size || 1) * 1.35
              } else if (edge === _focusedEdge) {
                newData.color = edgeHighlightColor
                newData.size = Number(data.size || 1) * 1.2
