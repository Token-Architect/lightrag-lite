...
export type QueryRequest = {
  ...
  enable_rerank?: boolean
  /** Whether to enable model-side thinking mode during retrieval generation. Default is false. */
...
export type MindMapMarkdownRequest = {
  max_docs?: number
  max_chars?: number
  instruction?: string
}

export type MindMapMarkdownResponse = {
...
axiosInstance.interceptors.request.use((config) => {
  ...
  if (workspace) {
    // XHR request headers only accept ISO-8859-1. Encode workspace to safely carry non-ASCII names.
    config.headers['LIGHTRAG-WORKSPACE'] = encodeURIComponent(workspace)
  } else {
...
export const queryTextStream = async (
  ...
  if (workspace) {
    headers['LIGHTRAG-WORKSPACE'] = encodeURIComponent(workspace);
  }
...
export const generateMindMapMarkdown = async (
  request: MindMapMarkdownRequest = {}
): Promise<MindMapMarkdownResponse> => {
  const response = await axiosInstance.post('/documents/mindmap_markdown', request)
  return response.data
}

export const listModelConfigs = async (): Promise<{ active_model_id: string | null; items: PlatformModelConfig[] }> => {
  const response = await axiosInstance.get('/platform/models')
  return response.data
}

export const createModelConfig = async (
  payload: Omit<PlatformModelConfig, 'id' | 'created_at' | 'updated_at' | 'binding'>
): Promise<{ status: string; data: PlatformModelConfig }> => {
  const response = await axiosInstance.post('/platform/models', payload)
  return response.data
}

export const deleteModelConfig = async (id: string): Promise<{ status: string; message: string }> => {
  const response = await axiosInstance.delete(`/platform/models/${encodeURIComponent(id)}`)
  return response.data
}

export const testModelConfig = async (id: string): Promise<{ status: string; message: string }> => {
  const response = await axiosInstance.post(`/platform/models/${encodeURIComponent(id)}/test`)
  return response.data
}

export const activateModelConfig = async (
  id: string
): Promise<{ status: string; active_model_id: string; apply_result?: { applied: boolean; message?: string } }> => {
  const response = await axiosInstance.post(`/platform/models/${encodeURIComponent(id)}/activate`)
  return response.data
}

export const getGraphVersionStatus = async (): Promise<any> => {
  const response = await axiosInstance.get('/platform/graph/version')
  return response.data
}

export const saveGraphVersion = async (note?: string): Promise<any> => {
  const response = await axiosInstance.post('/platform/graph/version/save', { note })
  return response.data
}

export const rollbackGraphVersion = async (): Promise<any> => {
  const response = await axiosInstance.post('/platform/graph/version/rollback')
  return response.data
}
