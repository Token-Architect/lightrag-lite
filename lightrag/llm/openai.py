...
def _is_ark_openai_compatible_base_url(base_url: str | None) -> bool:
    """Check whether the base URL looks like Volcengine Ark OpenAI-compatible endpoint."""
    if not base_url:
        return False
    normalized = base_url.lower().rstrip("/")
    return "volces.com" in normalized and "/api/v3" in normalized


async def _ark_multimodal_text_embed(
    texts: list[str],
    model: str,
    base_url: str,
    api_key: str | None,
    ...
    """
    Fallback for Ark multimodal embedding endpoint.

    It encodes each text using:
    ...
    def _decode_vector_payload(vector_obj: Any) -> np.ndarray | None:
        """Decode common vector payload formats to float32 ndarray."""
        if vector_obj is None:
            return None
        if isinstance(vector_obj, np.ndarray):
            return vector_obj.astype(np.float32)
        if isinstance(vector_obj, list):
            return np.array(vector_obj, dtype=np.float32)
        if isinstance(vector_obj, str):
            try:
                return np.frombuffer(base64.b64decode(vector_obj), dtype=np.float32)
            except Exception:
                return None
        if isinstance(vector_obj, dict):
            # Common wrappers seen in provider-compatible responses
            for key in ("embedding", "vector", "values", "float", "fp32", "dense"):
                decoded = _decode_vector_payload(vector_obj.get(key))
                if decoded is not None:
                    return decoded
        return None

    def _extract_vectors_from_response(data: Any) -> list[np.ndarray]:
        """Extract vectors from flexible Ark multimodal response schemas."""
        candidates: list[Any] = []
        ...
        for candidate in candidates:
            if isinstance(candidate, list) and candidate and isinstance(
                candidate[0], (int, float)
            ):
                vectors.append(np.array(candidate, dtype=np.float32))
                continue
            if isinstance(candidate, list):
                # Could be list of vectors / list of wrapped dicts
                for sub in candidate:
                    decoded = _decode_vector_payload(sub)
                    if decoded is not None and decoded.size > 0:
                        vectors.append(decoded)
                continue
            decoded = _decode_vector_payload(candidate)
            if decoded is not None and decoded.size > 0:
                vectors.append(decoded)
        return vectors
    ...
    # Batch all texts in one request to align with multimodal API behavior.
    payload = {
        "model": model,
    ...
    if token_tracker and isinstance(data, dict):
        usage = data.get("usage", {}) or {}
        total_prompt_tokens += int(
            usage.get("prompt_tokens", usage.get("input_tokens", 0)) or 0
        )
        total_tokens += int(usage.get("total_tokens", usage.get("output_tokens", 0)) or 0)

    vectors = _extract_vectors_from_response(data)
    if not vectors:
        if isinstance(data, dict):
            # Structured hint for debugging provider schema differences
            data_keys = list(data.keys())
            data_type = type(data.get("data")).__name__
            sample = repr(data.get("data"))[:300]
            raise ValueError(
                "Ark multimodal embedding response has no recognizable vector field. "
                f"top_keys={data_keys}, data_type={data_type}, data_sample={sample}"
            )
        raise ValueError(
            "Ark multimodal embedding response has no recognizable vector field."
        )

    # Best effort alignment: providers may return single vector or one per input.
    if len(vectors) == 1 and len(texts) > 1:
        vectors = vectors * len(texts)
    if len(vectors) < len(texts):
        raise ValueError(
            f"Ark multimodal embedding returned {len(vectors)} vectors for {len(texts)} inputs."
        )
    vectors = vectors[: len(texts)]
...
async def openai_complete_if_cache(
    ...
    **kwargs: Any,
    ...
    # Ark (Volcengine) supports model-side thinking control via extra_body.thinking.
    # Default follows enable_cot: False -> disabled, True -> enabled.
    if _is_ark_openai_compatible_base_url(base_url):
        extra_body = kwargs.get("extra_body")
        if not isinstance(extra_body, dict):
            extra_body = {}
        thinking = extra_body.get("thinking")
        if not isinstance(thinking, dict) or "type" not in thinking:
            extra_body["thinking"] = {"type": "enabled" if enable_cot else "disabled"}
...
async def openai_embed(
    ...
    async with openai_async_client:
        ...
        try:
            ...
            err_text = str(e)
            if "does not support this api" in err_text.lower() or (
                "invalidparameter" in err_text.lower() and "model" in err_text.lower()
            ):
                # Ark special fallback: vision embedding endpoints may only expose
                # /embeddings/multimodal while still supporting text items.
                if not use_azure and _is_ark_openai_compatible_base_url(base_url):
                    logger.warning(
                        "OpenAI-compatible /embeddings rejected model '%s'; "
                        "fallback to Ark /embeddings/multimodal with text input.",
                        api_model,
                    )
                    return await _ark_multimodal_text_embed(
                        texts=texts,
                        model=api_model,
                ...
                raise ValueError(
                    "Embedding model is not compatible with text /embeddings API. "
                    f"Current model='{api_model}'. "
                    "Please update .env EMBEDDING_MODEL to a TEXT embedding model/endpoint "
                    "(for Ark/OpenAI-compatible providers, use a text-embedding endpoint id, "
                    "not a vision embedding endpoint)."
                ) from e
            raise
