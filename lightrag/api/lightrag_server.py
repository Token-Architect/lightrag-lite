...
from lightrag.api.routers.platform_routes import create_platform_routes
from lightrag.api.routers.ollama_api import OllamaAPI

from lightrag.utils import logger, set_verbose_debug, priority_limit_async_func_call
...
def create_app(args):
    ...
    api_version_display = (
        f"{__api_version__}鈿狅笍" if is_frontend_outdated else __api_version__
    )
    ...
    workspace_instances: dict[str, tuple[LightRAG, DocumentManager]] = {}
    workspace_instances_lock = asyncio.Lock()

    current_rag_ctx: ContextVar[LightRAG | None] = ContextVar(
        "current_rag_ctx", default=None
    )
    current_doc_manager_ctx: ContextVar[DocumentManager | None] = ContextVar(
        "current_doc_manager_ctx", default=None
    )
    ...
    async def lifespan(app: FastAPI):
        ...
        try:
            ...
            # Clean up database connections for all initialized workspace instances
            finalized_ids = set()
            async with workspace_instances_lock:
                instances = list(workspace_instances.values())
            for rag_instance, _ in instances:
                instance_id = id(rag_instance)
                if instance_id in finalized_ids:
                    continue
                finalized_ids.add(instance_id)
    ...
    # Initialize FastAPI
    base_description = (
        "Providing API for Graph KB core, Web UI and Ollama Model Emulation"
    )
    swagger_description = (
        base_description
    ...
    def get_workspace_from_request(request: Request) -> str | None:
        ...
        if workspace:
            # Frontend URL-encodes header value to support non-ASCII workspace names.
    ...
    async def workspace_context_middleware(request: Request, call_next):
        ...
        rag_instance, doc_manager_instance = await get_or_create_workspace_instance(
            workspace
        )
        rag_token = current_rag_ctx.set(rag_instance)
        doc_manager_token = current_doc_manager_ctx.set(doc_manager_instance)
        try:
            return await call_next(request)
        finally:
            current_rag_ctx.reset(rag_token)
            current_doc_manager_ctx.reset(doc_manager_token)
    ...
    def build_rag_for_workspace(workspace: str) -> LightRAG:
        return LightRAG(
    ...
    async def get_or_create_workspace_instance(
        workspace: str | None,
    ) -> tuple[LightRAG, DocumentManager]:
        ws = normalize_workspace(workspace)
        existing = workspace_instances.get(ws)
        ...
        async with workspace_instances_lock:
            ...
            if ws == normalize_workspace(args.workspace):
                rag_instance = rag
                doc_manager_instance = doc_manager
            else:
                rag_instance = build_rag_for_workspace(ws)
                doc_manager_instance = DocumentManager(args.input_dir, workspace=ws)
                await rag_instance.initialize_storages()
                await rag_instance.check_and_migrate_data()
                logger.info(f"Initialized workspace instance: '{ws or '-'}'")

            workspace_instances[ws] = (rag_instance, doc_manager_instance)
            return workspace_instances[ws]

    class WorkspaceRAGProxy:
        def __getattr__(self, name):
            active = current_rag_ctx.get() or rag
            return getattr(active, name)

    class WorkspaceDocManagerProxy:
        def __getattr__(self, name):
            active = current_doc_manager_ctx.get() or doc_manager
            return getattr(active, name)

    # Initialize default RAG with unified configuration
    try:
        rag = build_rag_for_workspace(normalize_workspace(args.workspace))
    except Exception as e:
        logger.error(f"Failed to initialize LightRAG: {e}")
        raise

    default_ws = normalize_workspace(args.workspace)
    workspace_instances[default_ws] = (rag, doc_manager)

    async def apply_model_runtime_config(model_config: dict) -> dict:
        """
        Apply a selected model config to current runtime (LLM side).
        This updates args/rag in-memory so subsequent requests use the new provider/url/key/model.
        """
        provider = (model_config.get("provider") or "").strip().lower()
        binding = (model_config.get("binding") or "openai").strip().lower()
        model_name = (model_config.get("model_name") or "").strip()
        base_url = model_config.get("base_url")
        api_key_value = model_config.get("api_key")

        if not model_name:
            return {"applied": False, "message": "model_name is required"}

        old_binding = args.llm_binding
        old_model = args.llm_model

        # Normalize provider aliases to OpenAI-compatible binding.
        if provider in {"volcengine", "tongyi", "zhipu", "vllm", "openai_compatible"}:
            binding = "openai"

        args.llm_binding = binding
        args.llm_model = model_name
        if base_url:
            args.llm_binding_host = base_url
        if api_key_value is not None:
            args.llm_binding_api_key = api_key_value

        try:
            # Refresh config cache (captures provider-specific options).
            nonlocal config_cache
            config_cache = LLMConfigCache(args)

            llm_func = create_llm_model_func(args.llm_binding)
            llm_kwargs = create_llm_model_kwargs(args.llm_binding, args, llm_timeout)

            rag.llm_model_name = args.llm_model
            rag.llm_model_kwargs = llm_kwargs
            rag.llm_model_func = priority_limit_async_func_call(
                rag.llm_model_max_async,
                llm_timeout=rag.default_llm_timeout,
                queue_name="LLM func",
            )(
                partial(
                    llm_func,
            ...
            # Rollback in-memory args on failure.
            args.llm_binding = old_binding
            args.llm_model = old_model
            logger.error(f"Failed to apply model runtime config: {e}")
            return {"applied": False, "message": str(e)}

    rag_proxy = WorkspaceRAGProxy()
    doc_manager_proxy = WorkspaceDocManagerProxy()
    ...
    app.include_router(
        create_platform_routes(
            ...
            working_dir=args.working_dir,
            api_key=api_key,
            apply_model_config_cb=apply_model_runtime_config,
        )
    )
    ...
    async def get_status(request: Request):
        ...
        try:
            ...
            try:
                ...
                # Auto-heal per-workspace pipeline namespace for newly selected workspace.
